...
###################################################################################
## Check for cloudflare-acme.json file
###################################################################################
# Check if cloudflare-acme.json file exists in /data
echo -e "${green}Checking for cloudflare-acme.json file in /data...${reset}"
cd $DOCKER_COMPOSE_DIR/data/certs

if [ ! -f "cloudflare-acme.json" ]; then
  echo -e "${red}cloudflare-acme.json file not found in /data/certs. Creating...${reset}"
  touch cloudflare-acme.json
  echo -e "${green}cloudflare-acme.json file created successfully.${reset}"
fi

# Check and set permissions to 600 if not already set
if [ "$(stat -c '%a' cloudflare-acme.json)" != "600" ]; then
  echo -e "${green}Setting permissions to 600...${reset}"
  sudo chmod 600 cloudflare-acme.json
...
# Load CF_DNS_API_TOKEN from cf_api_token.txt if not already set
if [ -z "$CF_DNS_API_TOKEN" ] && [ -f "cf_api_token.txt" ]; then
  echo -e "${green}Loading CF_DNS_API_TOKEN from cf_api_token.txt...${reset}"
  export CF_DNS_API_TOKEN=$(cat cf_api_token.txt)
fi
